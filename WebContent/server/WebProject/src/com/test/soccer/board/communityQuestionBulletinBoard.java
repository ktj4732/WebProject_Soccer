...
@WebServlet("/board/communityQuestionBulletinBoard.do")
public class communityQuestionBulletinBoard extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (n == 1) {
			...
			pagebar += "<li>";
			if (search == null) {
			pagebar += String.format("<a href=\"/soccer/board/communityQuestionBulletinBoard.do?page=%d&selectrow=%s\" aria-label=\"Previous\">", n - 1, selectrow);
			} else {
			pagebar += String.format("<a href=\"/soccer/board/communityQuestionBulletinBoard.do?page=%d&selectKeyword=%s&search=%s&selectrow=%s\" aria-label=\"Previous\">", n - 1, selectKeyword, search, selectrow);				
			}			
			pagebar += "<span aria-hidden=\"true\">&laquo;</span>";
			pagebar += "</a>";
			pagebar += "</li>";
		...
		while(!(loop > blockSize || n > totalPage)) { //n > totalPage 18페이지 까진데 19 20 은 x
			...
			if (n == nowPage) {
				...
				pagebar += "<li>";
				if (search == null) {
				pagebar += String.format("<a href=\"/soccer/board/communityQuestionBulletinBoard.do?page=%d&selectrow=%s\">%d</a>", n, selectrow, n);
				} else {
				pagebar += String.format("<a href=\"/soccer/board/communityQuestionBulletinBoard.do?page=%d&selectKeyword=%s&search=%s&selectrow=%s\">%d</a>", n, selectKeyword, search, selectrow, n);
				}
				pagebar += "</li>";	
			}
			loop++;
		...
		if (n > totalPage) {
			...
			pagebar += "<li>";			
			if (search == null) {
			pagebar += String.format("<a href=\"/soccer/board/communityQuestionBulletinBoard.do?page=%d&selectrow=%s\" aria-label=\"Next\">", n, selectrow);
			} else {
			pagebar += String.format("<a href=\"/soccer/board/communityQuestionBulletinBoard.do?page=%d&selectKeyword=%s&search=%s&selectrow=%s\" aria-label=\"Next\">", n, selectKeyword, search, selectrow);																	
			}
			pagebar += "<span aria-hidden=\"true\">&raquo;</span>";
			pagebar += "</a>";
			pagebar += "</li>";
